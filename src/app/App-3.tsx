...
// ── Mock pre-filled certification data ────────────────────────────────────────
...
export default function App() {
  ...
  // ── Certification states (default 'rejected' for demo visibility) ────────────
  ...
  const handleLoginSuccess = (userId: string, info: any) => {
    const newUserInfo: UserInfo = {
      userId, nickname: info.nickname, avatar: info.avatar,
  ...
  const handleLogout = () => {
    ...
    setUserRole('first-visit'); setCurrentPage('order');
    localStorage.removeItem('userInfo'); localStorage.removeItem('userRole');
  };

  const requireLogin = (action: () => void) => {
    if (!isLoggedIn) { setPendingAction(() => action); setShowAuthModal(true); }
    else action();
  };

  const handleNavigateToMyOrders = () => {
    if (!isLoggedIn) { setPendingAction(() => () => setCurrentPage('myOrders')); setShowAuthModal(true); }
    else setCurrentPage('myOrders');
  };

  const handleNavigateToMyBills = () => {
    if (!isLoggedIn) { setPendingAction(() => () => setCurrentPage('myBills')); setShowAuthModal(true); }
    else setCurrentPage('myBills');
  };

  const handleNavigateToMessages = () => {
    if (!isLoggedIn) { setPendingAction(() => () => setCurrentPage('messageCenter')); setShowAuthModal(true); }
    else setCurrentPage('messageCenter');
  };
  ...
  return (
    <div className="size-full bg-white relative">
      ...
      {currentPage === 'order' ? (
        <OrderSquare
          userRole={userRole} setUserRole={handleSetUserRole}
          onNavigateToMyOrders={handleNavigateToMyOrders}
          isLoggedIn={isLoggedIn} requireLogin={requireLogin}
        />
      ) : currentPage === 'myOrders' ? (
        <MyOrders userRole={userRole} setUserRole={handleSetUserRole} onBack={() => setCurrentPage('order')} />
      ...
      <AdminEntrance onNavigate={setCurrentPage} currentPage={currentPage} />
    </div>
  );
}