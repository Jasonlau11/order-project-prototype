...
3. **定义方配置**：定义方在里程碑确认页面配置**总工期**及**各里程碑**：里程碑名称/摘要、**目标时间**（期望完成日期，用于提前提醒用户提交）、**最晚达成时间**（该日期前未达成则可能触发超期与客户终止权，可与约定完成日或约定完成日+宽限期一致）、交付说明/交付物要求等；支持自定义数量与内容。配置完成后保存，此时里程碑处于**待用户确认**状态。  
4. **用户确认**：用户（接单方）在订单详情或里程碑确认入口查看里程碑配置，确认无误后执行「确认」操作；用户确认后里程碑**生效**（配置状态为已生效），后续按 2.3 章节进行节点提交与客户验收。  
5. **保存与生效**：仅定义方保存时里程碑未生效，需用户确认后才生效；双方确认后，里程碑与总工期与该订单绑定，后续节点流程按已确认的里程碑执行。
...
- **数据影响**：里程碑主表或子表（关联订单）、总工期存储；与订单结算模式、订单状态关联。  
- **业务规则影响**：按结算模式确定里程碑与总工期的定义方（客户或平台运营方）；支持自定义数量与内容。  
- **历史文档查阅记录**：同 2.1.2；本功能对应整体方案 6.5.2「里程碑定义」「根据订单的结算模式，由客户或平台运营方与用户共同确认里程碑与总工期」。
